...
export async function GET() {
  try {
    ...
    if (!weatherRes.ok || !airRes.ok || !fxRes.ok) throw new Error("Failed to fetch external data");
    ...
    // open.er-api.com: base=THB, rates.JPY = how many JPY per 1 THB
    const thbToJpy = Number(fx?.rates?.JPY || 0);
    const jpyToThb = thbToJpy > 0 ? 1 / thbToJpy : 0;
    ...
    return success({
      ...
      exchange: {
        ...
        reverse: `1 JPY = ${jpyToThb.toFixed(3)} THB`,
      },
      events: events.map(e => ({
        id: e.id,
        time: e.eventTime,
